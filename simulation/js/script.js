...
document.addEventListener("DOMContentLoaded", function () {
  ...
  setTimeout(function () {
    const text =
      "Welcome to this Pharmacy laboratory experiment Particle Size Distribution by Microscopic Method ";
    const utterance = new SpeechSynthesisUtterance(text);
    speechSynthesis.speak(utterance);
  }, 0);
    ...
    timeline
      ...
      .to(singlepiece, {
        ...
        onComplete: function () {
          ...
          updateInstruction(
            "Eye piece attached. Now click on the stageometer to prepare the calibrate eyepiece",
          );
        },
      });
  });
  ...
  stageometerScale.addEventListener("click", () => {
    ...
    if (!experimentState.singlePiecePlaced) {
      updateInstruction(
        "You need to place the eyepiece on the microscope first. Click on the eyepiece.",
      );
      return;
    }
    ...
    timeline
      ...
      .to(stageometerScale, {
        ...
        onComplete: function () {
          ...
          updateInstruction(
            "Now click on the single piece to attach it to the microscope",
          );
        },
      });
  });
  ...
  next1.addEventListener("click", () => {
    ...
    if (!experimentState.stageometerPlaced) {
      updateInstruction(
        "You need to place the stageometer scale first. Click on the stageometer scale.",
      );
      return;
    }
    //  function nextPage() {
  ...
  pucki.addEventListener("click", () => {
    ...
    if (!experimentState.singlePiecePlaced) {
      updateInstruction(
        "You need to attach the eyepiece first. Click on the eyepiece.",
      );
      return;
    }
    ...
    if (!experimentState.nextButton) {
      updateInstruction(
        "You need to calibrate stagometer first. Click on the Next button.",
      );
      return;
    }
    ...
    timeline
      ...
      .to(glass2, {
        ...
        onComplete: function () {
          ...
          updateInstruction(
            "Sample added to slide. Click on the slide to place it on the microscope",
          );
        },
      })
      .to(glass, {
  ...
  previousglass.addEventListener("click", () => {
    if (!experimentState.singlePiecePlaced) {
      updateInstruction(
        "You need to attach the single piece first. Click on the single piece.",
      );
      return;
    }
    ...
    if (!experimentState.pucki) {
      updateInstruction(
        "You need to click on tube to prepare sample. Click on the Dropper.",
      );
      return;
    }
    ...
    if (!experimentState.samplePrepared) {
      updateInstruction(
        "You need to prepare the sample first. Click on the Dropper.",
      );
      return;
    }
  });
    ...
    timeline
      ...
      .to(glass2, {
        ...
        onComplete: function () {
          ...
          updateInstruction(
            "Calibration complete! Click 'next' to continue to the next step",
          );
        },
      });
  });
  ...
  next2.addEventListener("click", () => {
    ...
    function nextPage() {
      ...
      if (!experimentState.slidePlaced) {
        alert(
          "Please complete all the steps before proceeding to the next page.",
        );
        updateInstruction(
          "You need to complete all steps before proceeding. Make sure to place the eyepiece, stageometer scale, single piece, and prepare the sample with the slide.",
        );
        return;
      }
    }
